...
namespace TOR_ChanceModifier {
    public class ChanceModUpdater : MonoBehaviour {
        public const string RepositoryOwner = "DaUnknown-0";
        public const string RepositoryName = "TOR-Chance";
        public const string PluginAssetName = "TOR-ChanceModifier.dll";

        public static ChanceModUpdater Instance { get; private set; }

        public ChanceModUpdater(IntPtr ptr) : base(ptr) { }

        private bool _busy;
        private bool _showPopUp = true;
        public System.Collections.Generic.List<GithubRelease> Releases;
        ...
        private IEnumerator CoDownloadRelease(GithubRelease release, bool managerMode) {
            ...
            if (asset == null) {
                ...
                ChancePlugin.Logger?.LogError("[Chance] Update failed: release asset not found.");
                _updateState = 3;
                if (!managerMode) {
            ...
            try {
                ...
                ChancePlugin.Logger?.LogError($"[Chance] Update failed: could not move the old plugin file aside ({e.Message}).");
                _updateState = 3;
                if (!managerMode) {
                    popup.TextAreaTMP.text = "Update failed";
                    button.SetActive(true);
            ...
            if (!hasError) {
                _updateState = 2;
                if (!managerMode) {
                    popup.TextAreaTMP.text = "Chance Modifier\nupdated successfully\nPlease restart the game.";
                }
            } else {
                // ROLL BACK. The working DLL was moved aside to .old before the download was written,
                // so a failed write used to leave the plugin folder with no usable Chance at all (a
                ...
                try {
                    if (moved && File.Exists(filePath + ".old")) {
                        ...
                        ChancePlugin.Logger?.LogWarning("[Chance] Update failed - restored the previous plugin file.");
                    ...
                    ChancePlugin.Logger?.LogError(
                        $"[Chance] Update failed AND the previous plugin file could not be restored "
                        + $"({e.Message}). Reinstall Chance manually: the working DLL is next to it, "
        ...
        private void OnSceneLoaded(Scene scene, LoadSceneMode mode) {
            ...
            if (latestRelease == null || !IsActualUpdate(latestRelease.Version, ChancePlugin.Version) || !latestRelease.Assets.Any(FilterPluginAsset))
                return;
            ...
            string t = "Update Chance";
            StartCoroutine(Effects.Lerp(0.1f, (Action<float>)(p => text.SetText(t))));
            passiveButton.OnMouseOut.AddListener((Action)(() => text.color = Color.yellow));
        ...
        [HideFromIl2Cpp]
        public IEnumerator CoShowAnnouncement(string announcement, bool show = true, string shortTitle = "Chance Update", string title = "", string date = "") {
            ...
            Announcement chanceAnnouncement = new() {
                Id = "chanceAnnouncement",
                Language = 0,
                Number = 6970,
                Title = title == "" ? "Chance Modifier Announcement" : title,
            ...
            mgr.StartCoroutine(Effects.Lerp(0.1f, new Action<float>((p) => {
                if (p == 1) {
                    ...
                    popUp.Init(false);
                    DataManager.Player.Announcements.SetAnnouncements(new Announcement[] { chanceAnnouncement });
                    popUp.CreateAnnouncementList();
                    popUp.UpdateAnnouncementText(chanceAnnouncement.Number);
        ...
        public bool HasUpdate() {
            ...
            return t != null && t.Assets.Any(FilterPluginAsset)
                && IsActualUpdate(t.Version, ChancePlugin.Version);
        }
        ...
        public void TriggerChannelSwitch(bool stable) {
            ...
            if (r != null && SemCompare(r.Version, ChancePlugin.Version) != 0)
